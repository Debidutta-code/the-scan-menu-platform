...
export const ManagerSettings: React.FC = () => {
  ...
  const [whiteLabelPrimary, setWhiteLabelPrimary] = useState('#111827');
  const [whiteLabelSecondary, setWhiteLabelSecondary] = useState('#FFFFFF');
  const [whiteLabelBg, setWhiteLabelBg] = useState('#FFFFFF');
  const [whiteLabelText, setWhiteLabelText] = useState('#111827');
  const [whiteLabelFont, setWhiteLabelFont] = useState('Plus Jakarta Sans');
  ...
  const { data: whiteLabelData, refetch: refetchWhiteLabel } = useQuery({
    queryKey: ['whiteLabelConfig', activeRestaurantId],
    queryFn: async () => {
      const res = await apiClient.get(`/restaurants/${activeRestaurantId}/white-label`);
      return res.data;
    },
    enabled: !!activeRestaurantId && isEnabled('white_label'),
  });

  useEffect(() => {
    if (whiteLabelData?.success && whiteLabelData?.data) {
      const d = whiteLabelData.data;
      setWhiteLabelEnabled(!!d.enabled);
      setCustomDomain(d.customDomain || '');
      setWhiteLabelPrimary(d.primaryColor || '#111827');
      setWhiteLabelSecondary(d.secondaryColor || '#FFFFFF');
      setWhiteLabelBg(d.backgroundColor || '#FFFFFF');
      setWhiteLabelText(d.textColor || '#111827');
      setWhiteLabelFont(d.fontFamily || 'Plus Jakarta Sans');
      setWhiteLabelLogoUrl(d.logoUrl || '');
      setWhiteLabelFaviconUrl(d.faviconUrl || '');
      setHidePoweredBy(!!d.hidePoweredBy);
  ...
  const whiteLabelMutation = useMutation({
    mutationFn: async (payload: any) => {
      const res = await apiClient.patch(`/restaurants/${activeRestaurantId}/white-label`, payload);
      return res.data;
    },
    onSuccess: () => {
    ...
    onError: (err: any) => {
      toast(err.response?.data?.error?.message || 'Error updating White Label configuration', 'error');
    },
  });
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 pb-24 font-sans">
      <div className="mb-8">
        ...
        {/* White Label & Enterprise Branding Card */}
        {isEnabled('white_label') ? (
          <div className="bg-white rounded-3xl border border-slate-150 p-6 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h4 className="text-xs font-extrabold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                <Palette className="w-4 h-4 text-purple-600" strokeWidth={1.75} />
                <span>White Label & Enterprise Branding</span>
              </h4>
            ...
            <div className="space-y-4">
              <label className="flex items-center gap-3 p-4 border border-slate-200 rounded-2xl cursor-pointer hover:bg-slate-50 transition">
                <input
                  ...
                  className="h-4 w-4 rounded text-purple-600 accent-purple-600 border-slate-300"
                />
                <div>
                  <p className="text-sm font-bold text-slate-800">Enable Custom White-Label Branding</p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Apply dynamic brand themes, custom domains, and custom logos to public digital menus.
                  </p>
                </div>
              </label>
              ...
              {whiteLabelEnabled && (
                <div className="space-y-4 pt-2">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Custom Domain Hostname</label>
                      <input
                        type="text"
                        placeholder="menu.yourrestaurant.com"
                        value={customDomain}
                        onChange={(e) => setCustomDomain(e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:border-purple-500"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Brand Font Family</label>
                      <input
                        type="text"
                        placeholder="Inter, Outfit, Roboto"
                        value={whiteLabelFont}
                        onChange={(e) => setWhiteLabelFont(e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:border-purple-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={whiteLabelPrimary.startsWith('#') ? whiteLabelPrimary : '#111827'}
                          onChange={(e) => setWhiteLabelPrimary(e.target.value)}
                          className="h-9 w-12 rounded border border-slate-200 cursor-pointer"
                        />
                        <input
                          type="text"
                          value={whiteLabelPrimary}
                          onChange={(e) => setWhiteLabelPrimary(e.target.value)}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Secondary Color</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={whiteLabelSecondary.startsWith('#') ? whiteLabelSecondary : '#FFFFFF'}
                          onChange={(e) => setWhiteLabelSecondary(e.target.value)}
                          className="h-9 w-12 rounded border border-slate-200 cursor-pointer"
                        />
                        <input
                          type="text"
                          value={whiteLabelSecondary}
                          onChange={(e) => setWhiteLabelSecondary(e.target.value)}
                          className="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-xs font-mono"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={whiteLabelBg.startsWith('#') ? whiteLabelBg : '#FFFFFF'}
                          onChange={(e) => setWhiteLabelBg(e.target.value)}
                          className="h-9 w-12 rounded border border-slate-200 cursor-pointer"
                        />
                        <input
                          type="text"
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={whiteLabelText.startsWith('#') ? whiteLabelText : '#111827'}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Custom Logo URL</label>
                      <input
                        type="url"
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Favicon Image URL</label>
                      <input
                        type="url"
                        placeholder="https://yourrestaurant.com/favicon.ico"
                  ...
                  <label className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 transition">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-slate-800">Hide "Powered by The Scan Menu" Badge</p>
                      <p className="text-[11px] text-slate-500">Remove all platform attribution footers from customer menu screens.</p>
                    </div>
                  </label>

                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Custom CSS Overrides</label>
                    <textarea
                      rows={3}
                      placeholder="/* Enter custom CSS rules */"
                      value={customCss}
                      onChange={(e) => setCustomCss(e.target.value)}
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs font-mono bg-slate-900 text-slate-100 focus:outline-none focus:border-purple-500"
                    />
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      whiteLabelMutation.mutate({
                        enabled: whiteLabelEnabled,
                        customDomain,
                        primaryColor: whiteLabelPrimary,
                        secondaryColor: whiteLabelSecondary,
                        backgroundColor: whiteLabelBg,
                        textColor: whiteLabelText,
                        fontFamily: whiteLabelFont,
                        logoUrl: whiteLabelLogoUrl,
                        faviconUrl: whiteLabelFaviconUrl,
                        hidePoweredBy,
                        customCss,
                      })
                    }
                    disabled={whiteLabelMutation.isPending}
                    className="w-full py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-extrabold text-xs rounded-xl transition flex items-center justify-center gap-2 shadow-sm disabled:bg-purple-300"
                  >
          ...
          <div className="bg-slate-50 rounded-3xl border border-slate-200 p-6 opacity-75">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
              ...
              <span className="px-2.5 py-1 bg-slate-200 text-slate-600 text-[10px] font-black rounded-full uppercase tracking-wider">
                Enterprise Feature Gated
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-2">
              White Label custom domain, dynamic CSS branding, and powered-by badge removal are exclusive to the Enterprise tier.
            </p>
          </div>
        )}
