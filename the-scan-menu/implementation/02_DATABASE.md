...
## Core Models Overview (21 Collections)
...
- **RestaurantSettings**: Per-tenant payment provider configurations, tax rates, workflow modes, `inventoryConfig` (enable low stock alerts, threshold, auto-86 on zero stock), and custom branding.
...
- **FeatureFlag**: Per-restaurant feature flag toggles (`ordering`, `payments`, `waiter_call`, `kds`, `inventory`, `pos_integration`, etc.).
...
- **MenuItem**: Catalog item with price in paise/cents, availability (`isAvailable`), stock tracking fields (`trackStock`, `stockQuantity`, `lowStockThreshold`), diet badges, add-on options, and `externalIds` (e.g. `externalIds.petpooja`) for third-party POS item mapping.
- **Tax**: Individual tax definitions and compound tax group configurations.

### 3. Inventory Management (Phase 12 Addition)
- **InventoryLog**: Historical audit trail recording availability toggles, manual stock adjustments, atomic order decrements, and automated zero-stock 86s with timestamps, previous/new stock quantities, availability state, actor types (`MANAGER`, `STAFF`, `ORDER`, `SYSTEM`), and actor IDs.

### 4. Order Engine (Phase 8 Ordering Modes Expansion & Phase 10 POS Metadata)
...
### 5. Payments, Assistance & Integrations
...
2. **Catalog & Inventory Queries**:
   - `MenuItem`: `{ restaurantId: 1, isAvailable: 1, trackStock: 1 }` (Stock query & public menu availability index)
   - `InventoryLog`: `{ restaurantId: 1, menuItemId: 1, createdAt: -1 }` (Audit log pagination)
3. **Order Queues**: `Order` features compound indexes:
...
4. **POS Integration Logs**: `IntegrationSyncLog` features compound indexes:
