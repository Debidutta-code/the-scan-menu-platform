...
export const AdminRestaurants: React.FC = () => {
  ...
  const [staffModalRest, setStaffModalRest] = useState<Restaurant | null>(null);
  ...
  return (
    <div className="w-full space-y-8 font-sans">
                  ...
                  >
                    ...
                    <div className="border-t border-slate-100 pt-3 mt-4 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold text-slate-600">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <button
                          onClick={() => setStaffModalRest(rest)}
                          className="flex items-center gap-1.5 text-amber-700 hover:text-amber-950 bg-amber-50 hover:bg-amber-100 border border-amber-200/70 px-2 py-1 rounded-lg transition font-bold"
                          title="Manage Staff & Managers for this outlet"
                        >
                          <Users className="w-3.5 h-3.5 text-amber-600" strokeWidth={2} />
                          <span>Staff</span>
                        </button>

                        <button
                          onClick={() => setAssigningRestId(rest._id)}
                          className="flex items-center gap-1 text-slate-500 hover:text-slate-800 transition p-1"
                          title="Assign existing or new Manager"
                        >
      ...
      {/* Tenant Staff & Manager Management Modal */}
      {staffModalRest && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 z-50">
          <div className="bg-white rounded-3xl w-full max-w-5xl h-[90vh] shadow-2xl border border-slate-200 flex flex-col overflow-hidden">
            {/* Modal Header */}
            <div className="p-4 sm:p-5 border-b border-slate-150 flex items-center justify-between shrink-0 bg-slate-50/50">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-bold text-slate-900 font-display">
                      Staff & Managers — {staffModalRest.name}
                    </h3>
                  ...
                  <p className="text-xs text-slate-500">
                    Manage personnel, change passwords, and generate POS PINs for this restaurant outlet.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    navigate(`/admin/restaurants/${staffModalRest._id}?tab=staff`);
                    setStaffModalRest(null);
                  }}
                  className="hidden sm:flex items-center gap-1.5 text-xs font-bold text-slate-700 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-xl transition"
                  title="Open full configuration workstation"
                >
                  <span>Open Full View</span>
                  <ExternalLink className="w-3.5 h-3.5" />
                </button>
                <button
                  onClick={() => setStaffModalRest(null)}
            ...
            {/* Modal Body with ManagerStaff */}
            <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 bg-slate-50/30">
              <ManagerStaff restaurantId={staffModalRest._id} />
            </div>
          </div>
        </div>
      )}
