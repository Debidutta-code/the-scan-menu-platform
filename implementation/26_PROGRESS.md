...
- **Completed Date**: Today
- **Pull Request / Commit**: Auto-generated
- **Notes**: Successfully implemented Production Hardening & Infrastructure. Created `CacheService` singleton in-memory TTL cache with `set`, `get`, `del`, `invalidatePattern`, and `clear` methods. Hooked public menu caching (120s TTL, keyed by `public_menu_{restaurantId}`) into `PublicController.getMenu` and wired cache invalidation on all 8 menu write mutations in `menu.controller.ts` (create/edit/delete category/item, reorder, toggle availability, update stock). Implemented tiered rate limiting: `authRateLimiter` (10/15min on login), `orderPlacementRateLimiter` (30/min on public orders), `generalApiRateLimiter` (300/15min). Added `correlationIdMiddleware` using `crypto.randomUUID()` (no external dependency). Created `GET /health/liveness` and `GET /health/readiness` health probes (DB + memory metrics). Created `setupGracefulShutdown` with `SIGTERM`/`SIGINT` handling, 10-second force-exit guard, and proper Mongoose teardown. Registered all new middleware and routes in `index.ts`. Created `17_PRODUCTION_HARDENING.md` specification. 6/6 Vitest tests passing, TypeScript clean, ESLint clean.
- **Risks**: In-memory cache is process-local — cache is not shared across horizontal replicas. For multi-instance deployments, replace with Redis (`ioredis`) when scaling beyond a single process. Webhook dispatch remains synchronous (documented in Phase 15 risks).
