...
The core of the application. Services contain all the business rules, data transformation, and orchestration between different database models or external providers. Examples include `inventory.service.ts` (concurrency-safe atomic stock depletion, auto-86ing, and audit logging), `posIntegration.service.ts` (non-blocking POS sync dispatch), `notification.service.ts` (central Socket.io event emitter for KDS, inventory, and staff dashboards), `table.service.ts`, `token.service.ts`, `restaurantStats.service.ts`, and `email.service.ts`.
