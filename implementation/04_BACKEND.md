...
Act as orchestrators. They receive the HTTP request from the route, extract the necessary data (params, body, user context), call the appropriate Service layer functions, and then format the output using a standardized response envelope (`utils/response.ts`). Examples include `analytics.controller.ts` (summary, top items, peak hours overview), `kds.controller.ts` (active kitchen tickets, item status advancement, ticket bumping), `integration.controller.ts`, and `order.controller.ts`. **No business logic resides here.**

### 3. Services (`src/services/`)
The core of the application. Services contain all the business rules, data transformation, and orchestration between different database models or external providers. Examples include `analytics.service.ts` (read-only time-series metrics, revenue aggregation excluding cancelled/unpaid orders, top item availability badges, and local timezone peak hours), `inventory.service.ts` (concurrency-safe atomic stock depletion, auto-86ing, and audit logging), `posIntegration.service.ts` (non-blocking POS sync dispatch), `notification.service.ts` (central Socket.io event emitter for KDS, inventory, and staff dashboards), `table.service.ts`, `token.service.ts`, `restaurantStats.service.ts`, and `email.service.ts`.
